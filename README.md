...
| 报修 | 业主提交、物业派单、维修处理、业主确认与评价、允许状态下取消 |
| 缴费 | 查看账单、确认模拟缴费、生成参考号、同键重试保持幂等 |
| 车位 | 分步预约、重叠冲突检测、查看和取消预约 |
| 运营 | 物业首页展示待处理工单、已完成工单和待收费用 |
| AI | 报修整理、公告拟稿，人工采用后再提交 |

模拟缴费不发生真实扣款；离线 AI 按规则返回建议，可切换到配置的模型服务。这些是本次交付的功能范围。门禁硬件、停车硬件与真实支付网关不在交付范围内。
...
服务端同源托管页面与 API；Alembic 负责迁移，启动时写入幂等种子数据。11 张业务表包含社区关系约束和审计时间。模型调用只在服务端执行。
