...
def seed_database(session: Session) -> None:
    ...
    session.flush()
    session.add(ResidentHouse(id=1, community_id=1, user_id=1, house_id=1, relation_type="OWNER"))
    session.add_all(
        [
    ...
    session.flush()
    session.add(RepairEvent(id=1, community_id=1, repair_id=1, actor_id=1, action="CREATE", from_status=None, to_status="SUBMITTED", note="业主提交报修", created_at=now, updated_at=now))
    session.commit()
