...
- 代表性输出：pytest 覆盖 API 成功/错误 envelope、跨社区拒绝、合法/非法状态转换、SQLite 支付与车位真实并发、相邻/重叠预约、UTC 序列化及迁移漂移。
- Task 8 RED：静态托管/SPA/API 边界与 Alembic 空库测试合计 `1 passed, 4 failed`；安全重置另有 `2 failed`。
- Task 8 GREEN：聚焦生产测试 `8 passed`；最终完整后端 `48 passed`。

## 前端 TDD

- 代表性提示：用真实组件验证角色导航、登录恢复、首页信息数量、报修动作矩阵、车位 409 后选择保留和 URL 状态筛选。
- 代表性输出：Vitest 最终为 10 个测试文件、46 个测试；网络只在单元测试边界替代，页面与共享 provider 使用真实实现。
...
- 最终审查发现车位检查后写入竞态、认证查询缓存跨角色残留、SQLite 时区丢失、社区关系缺少数据库级复合约束及审计字段，均先观察 RED 再修复。
- 最终审查同时修复报修详情本地状态复用、JWT 异常边界、移动导航标题语义和 E2E 启动失败清理；迁移测试执行 Alembic `check` 并逐列核对类型、空值与默认值。
...
- 代表性输出：Playwright 覆盖报修全生命周期、丢失缴费响应后的同键重试、真实停车 409 与取消、公告发布后业主可见；最终 `1 passed (23.0s)`。
...
- 许可边界：`THIRD_PARTY_NOTICES.md` 明确本地 UI wrapper 为独立实现，列出直接依赖许可并保留 Unsplash 通知。
