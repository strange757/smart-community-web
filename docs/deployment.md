...
| 场景 | 入口 | 默认监听 |
|---|---|---|
| 现场演示，含离线 AI | `demo/run_demo.py` | `127.0.0.1:8000` |
...
启动器检查前端构建并启动两个服务。社区服务依次执行 Alembic 迁移、幂等种子数据和页面/API 托管。重复启动保留业务记录，不重复插入种子数据。

电脑访问 [系统首页](http://127.0.0.1:8000) 和 [API 文档](http://127.0.0.1:8000/docs)。离线 AI 只监听电脑的 `127.0.0.1:9100`；平板通过社区服务间接调用，无需映射 9100。
...
| AI 不可用 | 使用离线演示入口，或配置模型后重启；可继续手动填写 |
| 401 | 检查账号，退出后重新登录 |
